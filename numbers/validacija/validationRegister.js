...
// Nesto vise o regex-u mozete pogledati na sajtu https://regex101.com/. Ovde mozete da vidite kako se to koristi, kako se proverava neki string(slova), int(brojevi-celi brojevi bez decimalnog zapisa) u JS.
function validateRegistration() {
  ...
  if (!nameRegex.test(firstName)) {
    displayErrorMessage(
      "firstName",
      "Neispravno ime. Dozvoljena su samo slova, najmanje 3 karaktera."
    );
    return false;
  }
  ...
  if (!nameRegex.test(lastName)) {
    displayErrorMessage(
      "lastName",
      "Neispravno prezime. Dozvoljena su samo slova, najmanje 3 karaktera."
    );
    return false;
  }
  ...
  if (!emailRegex.test(email)) {
    displayErrorMessage("email", "Neispravna email adresa.");
    return false;
  }
  ...
  if (!passwordRegex.test(password)) {
    displayErrorMessage(
      "password",
      "Neispravan password. Minimum 6 karaktera, bar jedno veliko slovo i jedan specijalni karakter."
    );
    return false;
  }
  ...
  if (!phoneRegex.test(phoneNumber)) {
    displayErrorMessage(
      "phoneNumber",
      "Neispravan broj telefona. Mora sadržavati 10 cifara."
    );
    return false;
  }
...
function displayErrorMessage(inputId, errorMessage) {
  // Pronalazi element koji prikazuje poruku o grešci na osnovu ID-ja unosa,    PRONALAZI GRESKU
  const errorParagraph = document.getElementById(inputId + "Error");
  // Postavlja tekst poruke o grešci na pronađeni element,    PISE TEKSTOM GRESKU
  errorParagraph.textContent = errorMessage;
}

// Funkcija clearErrorMessages() BRISEE sve postojeće tekstualne poruke o grešci na formi.
function clearErrorMessages() {
  // Pronalazi sve elemente sa klasom 'text-danger' koji prikazuju poruke o grešci
  const errorMessages = document.querySelectorAll(".text-danger");   //PRONALAZI CRVENI TEKS
