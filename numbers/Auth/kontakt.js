...
function sendMail() {
  ...
  //izuku se podaci koje korisnik unese u formi i ako je sve dobro posalje nama na gmail
  ...
  emailjs
    ...
    .then((res) => {
      ...
      alert("Poruka je poslata uspešno!");
    })
...
function validirajFormu() {
  ...
  if (ime === "" || ime.length > 10 || !ime.match(/^[A-Z\s]+$/)) {
    document.getElementById("name-error").textContent =
      "Ime mora biti sve velikim slovima i najviše 10 karaktera.";
    isValid = false;
  } else {
    document.getElementById("name-error").textContent = "";
  }
  ...
  if (prezime === "" || prezime.length > 18 || !prezime.match(/^[A-Z\s]+$/)) {
    document.getElementById("lastname-error").textContent =
      "Prezime mora biti sve velikim slovima i najviše 18 karaktera.";
    isValid = false;
  } else {
    document.getElementById("lastname-error").textContent = "";
  }
  ...
  if (poruka.length < 100 || poruka.length > 250) {
    document.getElementById("message-error").textContent =
      "Poruka mora biti između 100 i 250 karaktera.";
    isValid = false;
  } else {
    document.getElementById("message-error").textContent = "";
  }
